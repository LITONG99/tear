...
class Saver:
    ...
    def save_checkpoint(self, args, trainer, epoch_itr, val_loss):
        ...
        if len(checkpoints) > 0:
            if not os.path.exists(checkpoints[0]):
...
def validate_and_save(args, trainer, task, epoch_itr, valid_subsets, end_of_epoch, saver):
    ...
    do_save = (
            (end_of_epoch and epoch_itr.epoch % args.save_interval == 0) # interval
            or num_updates >= max_update # stop
    )
    do_validate = (
                          (end_of_epoch and epoch_itr.epoch % args.validate_interval == 0)
    ...
    # Save checkpoint
    if epoch_itr.epoch<=args.min_train_epoch: pass # do not save in first 30 epochs, to accelerate
